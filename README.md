...
Я начал с создания нового элемента Panel в своем проекте. Я установил свойства для этого элемента следующим образом:

- BackColor: Indigo (использовал готовое значение цвета на вкладке "Интернет").
- Location: 12; 12
- Size: 99; 96
- Name: panelCell0_0

Это будет прототип клетки для нашего игрового поля, куда мы в дальнейшем будем ставить "крестик" или "нолик". Теперь моя задача — скопировать этот элемент 8 раз, чтобы в итоге получить 9 панелей, по три в каждом ряду. Для этого я воспользовался комбинацией клавиш Ctrl+C для копирования и Ctrl+V для вставки, а также использовал пункты "Копировать" и "Вставить" из контекстного меню.

Теперь у меня есть 9 клеток на игровом поле, готовых к дальнейшей разработке.
...
Для этого по очереди выделила каждую панель для клеток игрового поля и, аналогично тому как делала выше, создала методы-обработчики для событий Click, MouseEnter и MouseLeave.

Написала два новых метода ***CellMouseOver(object sender)*** и ***CellMouseOut(object sender)*** - они и будут выполнять основную работу по смене фона для каждой клетки поля, и их мы будем вызывать из методов-обработчиков для событий MouseEnter и MouseLeave для каждой клетки:
...
Далее мы создадим пустой метод ***FillCell(Panel panel, int row, int column)***, который будет в дальнейшем заполнять клетку игрового поля либо "крестиком", либо "ноликом", а также вызовем этот метод из каждого метода-обработчика события Click для всех клеток поля. При этом из каждого обработчика передаем координаты клетки - т.е. индекс ряда и столбца:
...
Создадим два общих метода для смены стиля кнопок:
...
Теперь, прежде чем перейдем к написанию самого игрового движка, нам осталось настроить видимость всех наших элементов управления игровым процессом, а также их расположение на главной форме.

В код формы добавла следующий метод **SetPlayersLabelsAndScoreVisible(bool visible)**, который будет отвечать за отображение/скрытие имён игроков, метки с указанием хода, а также кнопок "Сброс" и "Новая игра":
...
Он принимает единственный параметр visible, которым  задаём свойства Visible для всех нужных меток и панелей-кнопок.

Теперь выбрала главную форму FrmTicTacToe в конструкторе и двойным кликом по ней перейдем в созданный метод-обработчик события Load, т.е. первичной загрузки формы. Разместим там следующий код:
...
Здесь изначальные названия игроков будут равны некоторому неопределённому значению "?", а также вызываем метод SetPlayersLabelsAndScoreVisible со значением аргумента false, что говорит о том, что при загрузке главной формы лишние элементы управления будут изначально скрыты.
...
Игровой движок будет обеспечивать игровой процесс.

Для этого добавила к  проекту 2 новых класса и назвали их Cell и GameEngine. Добавить класс можно, кликнув правой кнопкой мыши на проекте TicTacToe и выбрав в контекстном меню  пункт Добавить → Класс..., как показано ниже:
...
У класса всего два целочисленных поля Row и Column, которые хранят индекс ряда и столбца в массиве игровых клеток (индексы начинаются с 0). У него есть параметризованный приватный конструктор Cell(int row, int column), который создаёт объект класса с заданными индексами ряда и столбца.
...
В коде класса GameEngine сразу оставим импорт пространства имён System и добавим необходимый импорт для пространства имён System.Drawing, которое нам пригодится в дальнейшем для работы с системными цветами. Остальные неиспользуемые импорты можно удалить.
...
Добавим в класс GameEngine два поля Mode и Turn и установим первичные значения режима игры и того, чей сейчас ход:
...
если параметр gameMode равен GameMode.None, то ничего не делаем и возвращаемся из метода. Если же передан правильный режим игры, то сбрасываем все счётчики с помощью вызова ResetScore(), устанавливаем режим движка Mode в значение, переданное при вызове метода, а далее, в зависимости от режима, в котором началась игра, устанавливаем текущий ход для 1-го игрока (т.к. у нас всегда начинает 1-й игрок, играющий за "крестики", независимо от режима игры).
...
Ход компьютера были разиты на три фазы или стратегии, которые будут применяться c каждым ходом компьютера, по очереди:
...
Поэтому написала универсальные методы, которые затем будем использовать как в атакующей, так и в защитной стратегиях хода компьютера:
...
Здесь два ключевых метода для осуществления компьютерного хода:
...
По выходу из цикла проверим - исчерпался ли счётчик попыток подбора. Если да, мы задействуем самую элементарную стратегию подбора свободной для хода клетки - просто бежим по рядам и столбцам по игровому полю до тех пор, пока не найдем свободную клетку (EMPTY_CELL) и её же вернем из метода.
...
Для завершения написания движка нам нужно реализовать в нём две последние вещи:
...
Добавим эту логику в игровой движок:
...
Последний метод для движка - IsWin(), который по очереди вызывает три предыдущих и возвращает true, если была победа, в противном случае возвращает false:
...
Теперь осталось лишь подключить и задействовать его в главной форме нашего приложения.

# **UI + GameEngine**. Подключаем готовый игровой движок к главной форме приложения
Возвратимся на главную форму FrmTicTacToe для игры. В самое начало класса добавляем новое поле engine класса GameEngine и инициализируем наш движок с помощью оператора new:
